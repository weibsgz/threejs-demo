...
scene.add(cube)
	...
	const texture = new THREE.TextureLoader().load('./textture/hot.png')
    const material = new THREE.SpriteMaterial({ map: texture })
    const sprite = new THREE.Sprite(material)
    // 设置大小、位置、内容
    sprite.scale.set(0.4,0.4,0.4)
    sprite.position.set(-4.5,-2,-2)
	sprite.name = 'sprite1'
    // 加入场景中
    scene.add(sprite)
...
document.body.addEventListener("click",function (event) {
	...
	const  isSprite1= intersects.filter((item)=>{
		return item.object.type == "Sprite" && item.object.name == 'sprite1'
	})
